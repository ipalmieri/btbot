...
class RpcDealer():
    """RPC message hub based on asyncio TCP server"""
    
    def __init__(self, listen_addr=None, port):
    ...
    async def _connect(self):
        logger.info(f"Starting server on {self.addr}:{self.serverport}")
        while not self.connected.is_set():
            try:
                con_coro = await self.aioloop.create_server(
